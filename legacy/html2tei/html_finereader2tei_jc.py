...
def replacingBasicElementsFromFineReader(text):
    ...
    text = re.sub(r'<p><span[^>]*>[ \*]+</span></p>', r'<milestone unit="section"/>', text, flags=re.DOTALL|re.IGNORECASE)
    ...
    text = re.sub(r'<(span|seg)[^>]*?>[:■— ~»\^\/\-\\\{\}\¿\'’£*f#/^•■~%]+</\1>', r'', text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'<br clear="all"/>', r'', text, flags=re.DOTALL|re.IGNORECASE|re.M)
    text = re.sub(r'<a .*?</a>', r'', text, flags=re.DOTALL|re.IGNORECASE|re.M)
    text = re.sub(r'<seg [^>]*?> *</seg>', r'', text, flags=re.DOTALL|re.IGNORECASE|re.M)
...
def cleaningTextualTyposOCR(text):
    ...
    text = re.sub(r'<p>—\-', r'<p>—', text, flags=re.IGNORECASE)
    text = re.sub(r'<p>— ?[1jiíl\|I;\/\'\[{] ?', r'<p>—¡', text)
    text = re.sub(r'<p> ?[1jiíl\|I;\/\'\[{] *([A-ZÁ-Ú])', r'<p>¡\1', text)
...
def main():
    ...
    for doc in glob.glob("input/*.html"):
        ...
        content = cleaningWhiteSpace(content)
