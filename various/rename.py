...
def transnomino(file,metadatafile):
    ...
    #print(metadatax)
    author = metadatax.loc[idno, 'sauthor']
    stitle = metadatax.loc[idno, 'stitle']
    genre = metadatax.loc[idno, 'sgenre']
    date = metadatax.loc[idno, 'date']
    #decade = metadatax.loc[idno, 'decade']
    #persp = metadatax.loc[idno, 'persp']    
    newfilename = "./derived/txt-sp-labels/" + author +"_"+ genre +"-"+idno+".txt"  # Supposes there is a subfolder called "output". 
...
main("./derived/txt-sp-labels/*.txt","./theatreclassique.csv") 							# USER: Enter absolute or relative path to folder with files, define pattern for filename (e.g., extension), enter path for metadata file.
